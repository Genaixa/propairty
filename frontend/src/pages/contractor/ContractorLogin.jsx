...
export default function ContractorLogin() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const res = await axios.post(`${BASE}/contractor/token`, params)
      localStorage.setItem('contractor_token', res.data.access_token)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-white">
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">Contractor Portal</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 pr-10"
        ...
        <div className="mt-6 rounded-xl border border-orange-100 bg-orange-50 px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold text-orange-700 uppercase tracking-wide">Demo access</p>
            <button
              type="button"
              onClick={() => { setEmail('info@negasservices.co.uk'); setPassword('demo1234') }}
              className="text-xs text-orange-600 hover:text-orange-800 font-medium hover:underline"
            >
              Fill in
            </button>
          </div>
          <p className="text-xs text-orange-600 font-mono">info@negasservices.co.uk</p>
          <p className="text-xs text-orange-500 font-mono">demo1234</p>
        </div>
